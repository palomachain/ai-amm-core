...
contract TestAiAmmRouter is IAiAmmSwapCallback {
    ...
    function aiAmmSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IAiAmmPool(msg.sender).token0() : IAiAmmPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IAiAmmPool(pools[0]).token1();
            IAiAmmPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
