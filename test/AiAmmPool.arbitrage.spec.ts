...
import { AiAmmPoolSwapTest } from '../typechain/AiAmmPoolSwapTest'
...
describe('AiAmmPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('AiAmmPoolSwapTest')
            const tester = (await testerFactory.deploy()) as AiAmmPoolSwapTest
