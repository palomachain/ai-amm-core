...
import { TestAiAmmSwapPay } from '../typechain/TestAiAmmSwapPay'
...
import { TestAiAmmReentrantCallee } from '../typechain/TestAiAmmReentrantCallee'
...
describe('AiAmmPool', () => {
  ...
  let factory: AiAmmFactory
  let pool: MockTimeAiAmmPool

  let swapTarget: TestAiAmmCallee
  ...
  async function initializeAtZeroTick(pool: MockTimeAiAmmPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestAiAmmReentrantCallee')
      ).deploy()) as TestAiAmmReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestAiAmmSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestAiAmmSwapPay')
      underpay = (await underpayFactory.deploy()) as TestAiAmmSwapPay
